...
**IAP test path (iPhone — no date change needed):**
1. Complete the in-app guide (8 pages).
2. Open the **Settings** tab (rightmost in the bottom tab bar).
3. In the **Full Access** (전체 기능) card at the top, tap **Unlock Full Access** (전체 기능 열기) — visible even during the 14-day trial.
4. Paywall sheet: purchase **Unlock forever for ₩7,900** or **Restore Purchases** (구매 복원).
5. Sandbox: sign in under iOS Settings → App Store → Sandbox Account, then purchase or restore.
...
6. **History** — completion calendar; **Settings** — trial status, IAP, appearance, guide replay.
...
- **IAP:** 설정 탭 → 「전체 기능」 → 「전체 기능 열기」(체험 중에도 표시) → 구매·복원.
